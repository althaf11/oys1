...
import { Landmark, FileText, Building2, Briefcase, BookOpen, Zap, Target } from 'lucide-react';
...
const GovernmentJobsPage = () => {
  ...
  const courses = [
    { name: "UPSC Preparation", icon: <Landmark className="w-6 h-6" />, description: "IAS, IPS, IFS exams" },
    { name: "SSC Exams", icon: <FileText className="w-6 h-6" />, description: "CGL, CHSL, MTS" },
    { name: "Banking Exams", icon: <Building2 className="w-6 h-6" />, description: "IBPS, SBI, RBI" },
    { name: "Railway Exams", icon: <Briefcase className="w-6 h-6" />, description: "RRB, NTPC" },
    { name: "State PSC", icon: <Landmark className="w-6 h-6" />, description: "State-level exams" }
  ];
  
  const coachingCenters = [
    "Civil Services Academy",
    "Government Exam Preparation Center",
    "Banking & SSC Institute",
    "Competitive Exam Coaching Hub",
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-7xl mx-auto">
        <a href="/career/job" className="text-blue-600 hover:text-blue-700 mb-6 inline-block no-underline">← Back to Job Categories</a>
        
        <ActionButtons buttons={actionButtons} />
        
        <div className="bg-white rounded-xl p-8 shadow-lg mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">Government Career Path</h1>
          <p className="text-gray-600 text-lg">
            Secure your future with government jobs through UPSC, SSC, Banking, Railways, and other competitive exams. Prepare systematically with expert guidance.
          </p>
        </div>
